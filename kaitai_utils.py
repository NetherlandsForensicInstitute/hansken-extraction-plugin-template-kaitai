...
def _get_kaitai_class():
    """
    Finds the Python file generated by Kaitai with the same name as the .ksy file, and returns the top-level class
    defined in it.
...
class _KaitaiToTraceWriter:
    ...
    def _process_bytes(self, value_object: Any, offsets: dict, path: str, key: str) -> tuple[str, Any]:
        """
        Method responsible for converting a byte array from the Kaitai object tree to a trace if necessary and / or
        putting it in the resulting JSON output.
        ...
        if len(value_object) > self.max_byte_array_length:
            if len(value_object) < hansken_extraction_plugin.runtime.constants.MAX_CHUNK_SIZE:
                ...
                if self.has_process and offsets is not None:
                    ...
                    child_builder.add_transformation('raw',
                                                     RangedTransformation([Range(offsets[key]['start'], length)]))
                    child_builder.build()
                    return _to_lower_camel_case(key), f'data block of size: {len(value_object)} (stored as {path})'
            else:
                return _to_lower_camel_case(key), (f'data block of size: {len(value_object)} (not added as child trace '
                                                   f'because size exceeds MAX_CHUNK_SIZE)')
        else:
